...
_executor = ThreadPoolExecutor(max_workers=settings.ai_max_workers)
...
def provider_available(provider: str, api_key: str = "") -> bool:
    """判断某供应商是否可用：优先透传的用户密钥，其次后端环境变量。

    custom 还需配置 base_url 环境变量；dashscope 无 workspace_id 时回退公共端点。
    """
    key = (api_key or "").strip() or _provider_api_key(provider)
    if provider == "dashscope":
        # 无 workspace_id 时回退公共端点，也因此密钥本身即可判定可用
        return bool(key)
    if provider == "custom":
...
def _resolve_base_url(provider: str) -> str:
    """解析 OpenAI 兼容供应商的 base_url：千问有专署 workspace_id 用私有地址，否则用公共兼容端点；custom 直接取配置。"""
    if provider == "dashscope":
        workspace_id = settings.dashscope_workspace_id.strip()
        if workspace_id:
            # 百炼专署实例：按 workspace_id 拼接私有兼容端点
            return f"https://{workspace_id}.cn-beijing.maas.aliyuncs.com/compatible-mode/v1"
        # 标准公共兼容端点：普通 API Key 无需 workspace_id
...
async def _call_ai_api(prompt: str, provider: str = "zhipu", model: str = "", api_key: str = "") -> Dict:
    """按供应商分发调用：优先使用透传的用户密钥，其次后端环境变量。"""
    if provider not in SUPPORTED_PROVIDERS:
        raise ValueError(f"不支持的 AI 供应商: {provider}")

    # 有效密钥优先级：用户自配 > 后端环境变量
    api_key = (api_key or "").strip() or _provider_api_key(provider)
    if not api_key:
        env_name = PROVIDER_KEY_ENV[provider]
        logger.error("[AI Extractor] 未配置 %s 或用户自配密钥", env_name)
        raise ValueError(f"使用 {SUPPORTED_PROVIDERS[provider]} 提取前请先在后端环境变量 {env_name} 或页面中配置密钥")
...
网页内容:
{html_content[:settings.ai_max_content_len]}
...
{{
  ...
  "keywords": "关键词,关键词,关键词"
    ...
    result = await _call_ai_api(prompt=prompt, provider=provider, model=model, api_key=api_key)
...
只返回 JSON：{{"summary":"100-200字摘要","keywords":"关键词,关键词,关键词"}}"""
    ...
    try:
        result = loop.run_until_complete(_call_ai_api(prompt, provider, model, api_key))
        return _parse_json_response(result["content"].strip())
    finally:
        loop.close()
...
async def extract_article_summary(
    ...
    return await loop.run_in_executor(_executor, _extract_article_summary_sync, content, provider, model, api_key)
...
async def extract_article_async(article_id: int) -> bool:
    ...
    try:
        ...
        text_content = " ".join(soup.get_text(separator=" ", strip=True).split())[: settings.ai_max_text_len]
