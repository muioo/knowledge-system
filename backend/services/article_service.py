...
from backend.ai.ai_extractor import extract_article_from_url
...
async def create_article(
    ...
    Returns:
        创建的文章响应
    """
    from backend.storage.article_storage import save_html_content
    ...
    if import_type == "file":
        ...
        try:
            ...
            # 回滚：删除文章和文件
            await article.delete()
            from backend.storage.article_storage import delete_article_files
...
async def import_article_from_html_url(
    ...
    if use_ai:
        ...
        from backend.services.ai_setting_service import get_user_provider_model, get_user_api_key
        use_provider = provider
        use_model = model or ""
        if not use_provider:
            use_provider, use_model = await get_user_provider_model(author_id)
        if not use_provider:
            raise ValueError("服务端未配置任何 AI 供应商密钥，且您未在设置页配置密钥，无法使用 AI 提取")

        # 优先使用用户自配密钥（解密后透传），未配置则走服务端环境变量
        user_api_key = await get_user_api_key(author_id, use_provider)
...
async def get_article_html_content(article_id: int) -> str:
    ...
    """
    from backend.storage.article_storage import get_article_file_content
    from backend.settings.config import settings
