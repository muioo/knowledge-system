...
async def upgrade(db: BaseDBAsyncClient) -> str:
    ...
    KEY `idx_reading_goa_user_id_c76634` (`user_id`, `is_active`, `start_date`)
        ...
        ALTER TABLE `reading_history` ADD `is_completed` BOOL NOT NULL  COMMENT '是否已完成阅读（进度>=100%）' DEFAULT 0;
        ALTER TABLE `reading_history` ADD `total_content_length` INT NOT NULL  COMMENT '总内容长度（像素）' DEFAULT 0;
        ALTER TABLE `reading_history` ADD `created_at` DATETIME(6) NOT NULL  COMMENT '记录创建时间' DEFAULT CURRENT_TIMESTAMP(6);
        ALTER TABLE `reading_history` ADD `session_id` VARCHAR(100)   COMMENT '阅读会话ID，用于关联同一次阅读的多次暂停/继续';
        ALTER TABLE `reading_history` ADD `scroll_position` INT NOT NULL  COMMENT '滚动位置（像素）' DEFAULT 0;
        ALTER TABLE `reading_history` ADD `actual_progress` INT NOT NULL  COMMENT '实际阅读进度（基于滚动位置计算）' DEFAULT 0;
        ALTER TABLE `reading_history` ADD `device_type` VARCHAR(50)   COMMENT '设备类型（desktop/mobile/tablet）';
        ALTER TABLE `reading_history` ADD `ip_address` VARCHAR(50)   COMMENT 'IP地址';
        ALTER TABLE `reading_history` MODIFY COLUMN `started_at` DATETIME(6) NOT NULL  COMMENT '开始阅读时间';
        ALTER TABLE `reading_history` MODIFY COLUMN `reading_duration` INT NOT NULL  COMMENT '阅读时长（秒）' DEFAULT 0;
        ALTER TABLE `reading_history` MODIFY COLUMN `ended_at` DATETIME(6)   COMMENT '结束阅读时间';
        ALTER TABLE `reading_history` MODIFY COLUMN `reading_progress` INT NOT NULL  COMMENT '阅读进度（0-100）' DEFAULT 0;
    ...
    KEY `idx_reading_not_article_dda356` (`article_id`, `created_at`)
        ...
        ALTER TABLE `reading_stats` ADD `max_reading_progress` INT NOT NULL  COMMENT '最高阅读进度（0-100）' DEFAULT 0;
        ALTER TABLE `reading_stats` ADD `last_reading_progress` INT NOT NULL  COMMENT '最后阅读进度（0-100）' DEFAULT 0;
        ALTER TABLE `reading_stats` ADD `first_read_at` DATETIME(6)   COMMENT '首次阅读时间';
        ALTER TABLE `reading_stats` ADD `completed_reads` INT NOT NULL  COMMENT '完成阅读次数（进度>=100%）' DEFAULT 0;
        ALTER TABLE `reading_stats` ADD `avg_duration` INT NOT NULL  COMMENT '平均每次阅读时长（秒）' DEFAULT 0;
        ALTER TABLE `reading_stats` MODIFY COLUMN `total_views` INT NOT NULL  COMMENT '总阅读次数' DEFAULT 1;
        ALTER TABLE `reading_stats` MODIFY COLUMN `last_read_at` DATETIME(6) NOT NULL  COMMENT '最后阅读时间' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6);
        ALTER TABLE `reading_stats` MODIFY COLUMN `total_duration` INT NOT NULL  COMMENT '总阅读时长（秒）' DEFAULT 0;
