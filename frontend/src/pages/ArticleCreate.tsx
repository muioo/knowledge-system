...
import UrlImportFields from '../components/article/UrlImportFields';
interface FileData {
  file: File | null;
  images: File[];
  title: string;
...
const ArticleCreate: React.FC = () => {
  ...
  const handleUrlDataChange = (data: Partial<Omit<UrlData, 'tagIds'>>) => {
      ...
      <Card>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {mode === 'file' ? (
            ...
            <UrlImportFields data={urlData} onChange={handleUrlDataChange} />
          )}
          <div>
